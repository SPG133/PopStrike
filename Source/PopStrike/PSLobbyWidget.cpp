...
void UPSLobbyWidget::NativeConstruct()
{
    ...
    if (LobbyState) LobbyState->OnChanged.AddUniqueDynamic(this, &ThisClass::RefreshLobby);
    if (LobbyController) LobbyController->OnLobbyError.AddUniqueDynamic(this, &ThisClass::HandleLobbyError);
    RefreshLobby();
}

void UPSLobbyWidget::NativeDestruct()
{
    if (LobbyState) LobbyState->OnChanged.RemoveDynamic(this, &ThisClass::RefreshLobby);
    if (LobbyController) LobbyController->OnLobbyError.RemoveDynamic(this, &ThisClass::HandleLobbyError);
...
void UPSLobbyWidget::RefreshLobby()
{
    ...
    if (bHasMap) CurrentMap = LobbyState->Maps[LobbyState->SelectedMap];
    bIsHost = LobbyController && LobbyController->IsRoomHost();
    bCanInteract = LobbyState && !LobbyState->bStarting;
    BP_OnLobbyRefreshed();
}

void UPSLobbyWidget::JoinTeam(EPSLobbyTeam Team)
{
    if (LobbyController) LobbyController->ServerJoinTeam(Team);
}

void UPSLobbyWidget::ChangeMap(int32 Offset)
{
    if (!LobbyController || !LobbyState || LobbyState->Maps.IsEmpty()) return;
    const int32 Count = LobbyState->Maps.Num();
    LobbyController->ServerSelectMap((LobbyState->SelectedMap + Offset % Count + Count) % Count);
}

void UPSLobbyWidget::StartGame()
{
    if (LobbyController) LobbyController->ServerStartGame();
}
